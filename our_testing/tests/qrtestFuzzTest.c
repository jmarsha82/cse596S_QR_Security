...
// Takes the data and writes it to a temporary file
...
extern int LLVMFuzzerTestOneInput(const uint8_t *data, size_t data_len)
{
	...
	int i;
	decoder = quirc_new();
	// struct pfile f = { .buf = data, .size = data_len, .cur = 0 };
	...
	// FILE* file;
	// file = fopen ("/home/kail/QRCode/fuzz.png", "wb+");
	// if(file == NULL){
  ...
  // fwrite (data , sizeof(uint8_t),  data_len, file);
	...
	ret = loader(decoder, buf_to_file(data, data_len));
	(void)clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &tp);
